...
class TimesheetController extends Controller
{
    ...
    public function update_tache_traitement(Request $request)
    {
        ...
        if ($tache->user_id == Auth::id()) {
            ...
            return redirect('/timesheet_hr')->with('status', 'modification avec succés.');
        } 
        else 
        {
            return redirect('/timesheet_hr')->with('error', 'Vous n\'êtes pas autorisé à mettre à jour cette tâche.');
        }
    }
    ...
    public function delete_tache( $id)
    {
        $tache=Tache::find($id);
        $tache->delete();
        return redirect('/timesheet_hr')->with('status', 'suppression avec succés.');
    }
