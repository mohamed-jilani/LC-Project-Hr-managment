    ...
    <div class="row">
      ...
      <div class="col-lg-8 col-md-10 mx-auto">
        <div class="card mt-4">
          ...
          @foreach($congesHistory as $conge)
    <!--subject, created_at, finished_at-->
    <div class="alert alert-info alert-dismissible text-white" role="alert">
         @if($conge->etat == 1)
         <span class="text-sm">
           le demande de congés: {{$conge->subject}} de {{$conge->created_at}} jusqu'a {{$conge->finished_at}} a été validé
        </span>
        @else
        <span class="text-sm">
           le demande de congés: {{$conge->subject}} de {{$conge->created_at}} jusqu'a {{$conge->finished_at}} a été refusé
        </span>
        @endif
